...
public class GUIClipboard extends GUIPlugin
{
    ...
    private void initComponents()
    {
        ...
        //from here	
        JButton b8m = new JButton(actionManual);
        b8m.setToolTipText("Manual");
        b8m.setText("");
        editToolBar.add(b8m);
        //till here
    ...
    private void setupActions()
    {
    	...
    	//from here
    	actionManual = new AbstractAction()
    	{
    		public void actionPerformed(ActionEvent e)
    		{
    		    ...
    		    else
    		    {
    		    	...
    		    	try {
					     //	Process p = Runtime.getRuntime().exec(new String[]{"bash","-c","ls "+f.getAbsolutePath()});
						//p.waitFor();
        ...
        actionManual.putValue(Action.LONG_DESCRIPTION, "Open Manual.");
        actionManual.putValue(Action.MNEMONIC_KEY, new Integer(KeyEvent.VK_M));
        actionManual.putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_M, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
        actionManual.putValue(Action.NAME, "Manual");
        actionManual.putValue(Action.SMALL_ICON, GUIPrism.getIconFromImage("smallFileText.png"));
